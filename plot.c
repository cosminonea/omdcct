...
void *writecache(void *arguments) {
	...
	if(asyncmode == 1) {
		printf("%i Percent done. (ASYNC write)\n", percent);
		fflush(stdout);
	} else {
		printf("%i Percent done. (write)\n", percent);
		fflush(stdout);
	}
	...
	printf("%i Percent done. %i nonces/minute, %i:%02i left\n", percent, speed, h, m);
	fflush(stdout);
...
int main(int argc, char **argv) {
	...
	for(run = 0; run < nonces; run += staggersize) {
		...
		printf("Nonce %llu/%d(%llu%%) done...\n", run + 1, nonces, (run + 1) / nonces);
	}
